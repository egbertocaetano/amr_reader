...
transform = pinv(sigma).dot(U.T)

path_transform = '/home/forrest/workspace/LINE/Baselines/AMR/results/19-04-10__22-08-45__MSRParaphraseCorpus/matrix/transform_11542_components_U.npy'
...
for idx in range(0, length, 2):
    row_1 = doc_conc_mtx[idx].dot(transform.T)
    row_2 = doc_conc_mtx[idx + 1].dot(transform.T)

    # sum_rows = np.sum([row_1, row_2], axis=0)

    # absolute_value = np.absolute(np.subtract(row_1, row_2))

    new_vector = np.concatenate((np.sum([row_1, row_2], axis=0), np.absolute(np.subtract(row_1, row_2))))
